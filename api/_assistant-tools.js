...
// Ferramentas de LEITURA do assistente conversacional (Onda 1).
// Cada ferramenta tem: schema (para o Claude) + executor (roda server-side,
// usando o token Meta do servidor — o mesmo padrão do meta-proxy).
...
function presetFromPeriod(period) {
    switch (period) {
...
// Normaliza texto para casar nomes de conta ("cliente") sem depender de acentos/caixa.
...
// ─── Resolução de conta por nome ("cliente") ou ID ───────────────────────────
...
async function resolveAccount(query) {
    ...
    // 1) match exato por id (act_123 ou 123)
    let match = accounts.find(
        (a) => normalize(a.id) === q || normalize(a.account_id) === q,
    );
    // 2) nome exato
...
// ─── Definição das ferramentas (schema + executor) ───────────────────────────
    ...
    },
    {
        ...
        input_schema: {
            ...
            properties: {
                conta: {
                    type: 'string',
                    description: 'Nome do cliente ou ID da conta (ex: "Padaria do João" ou "act_123456").',
                },
                periodo: {
                    type: 'string',
                    enum: ['today', 'yesterday', '7d', '30d', 'month'],
                    description: 'Período das métricas. Default: 7d.',
                },
            },
            required: ['conta'],
        },
        execute: async ({ conta, periodo = '7d' }) => {
            ...
            if (!match) {
                return {
                    erro: `Nenhuma conta encontrada para "${conta}".`,
                    contas_disponiveis: accounts.map((a) => a.name),
                };
            }
            ...
            return {
                ...
                id: match.id,
                periodo,
                metricas: summarizeInsights(insights) || 'Sem dados no período.',
            };
        },
    },
    {
        ...
        input_schema: {
            ...
            properties: {
                conta: {
                    type: 'string',
                    description: 'Nome do cliente ou ID da conta.',
                },
                periodo: {
                    type: 'string',
                    enum: ['today', 'yesterday', '7d', '30d', 'month'],
    ...
    },
    {
        ...
        input_schema: {
            ...
            properties: {
                metrica: {
                    type: 'string',
                    enum: ['custo_por_lead', 'leads', 'investimento', 'ctr', 'cpm'],
                    description: 'Métrica usada para ordenar.',
                },
                ordem: {
                    type: 'string',
                    enum: ['desc', 'asc'],
                    description: 'desc = maior primeiro (default); asc = menor primeiro. Para custo_por_lead, asc lista os melhores primeiro.',
                },
                periodo: {
                    type: 'string',
                    enum: ['today', 'yesterday', '7d', '30d', 'month'],
                    description: 'Período. Default: 7d.',
                },
            },
            required: ['metrica'],
        },
...
// Schema enviado ao Claude (sem a função execute).
